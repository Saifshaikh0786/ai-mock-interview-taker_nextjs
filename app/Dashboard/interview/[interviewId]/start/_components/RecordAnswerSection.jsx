...
function RecordAnswerSection({ mockInterviewQuestion, activeQuestionIndex, interviewData }) {
  ...
  const SavedUserAnswer = async () => {
    if (isRecording) {
      ...
      Based on the question and user answer, provide:
      - Rating (1-10)
      - Feedback on strengths
      - Areas of improvement
      - Format response in JSON with 'rating' and 'feedback' fields only dont  anything extra at start and end only JSON format its warning.`;
      ...
      try {
        ...
        if (resp) {
          toast({
            ...
            description: "You can move to the next question or reattempt this one.",
            variant: "destructive",
          });
          setUserAnswer("");
          setResults([]);
          ...
          <div className="relative w-full h-full rounded-lg overflow-hidden bg-slate-900">
            ...
            {isRecording && (
              <div className="absolute inset-0 flex items-end justify-center">
                <div className="animate-pulse text-2xl font-bold text-red-500">● RECORDING</div>
              </div>
            )}
          </div>
        </div>
      ...
      >
        ...
        <div
          className={`absolute inset-0 -z-10 bg-gradient-to-b ${
            isRecording ? "from-red-500/10 to-transparent" : "from-blue-500/10 to-transparent"
